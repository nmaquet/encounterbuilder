...
describe("lootService", function () {
    ...
    describe("service.generateTypeCLoot", function () {
        ...
        for (var budgetLowerBound = 0; budgetLowerBound < 100000; budgetLowerBound += 1000) {
            ...
            it("should generate type C loot for budgets " + budgetLowerBound + " to " + (budgetLowerBound + 950), function () {

                for (var budget = budgetLowerBound; budget <= budgetLowerBound + 950; budget += 5) {
                    var loot = service.generateTypeCLoot(budget);
                    ...
                    expect(loot.items.length).to.be.at.least(1);
                    for (var i in loot.items) {
                        var item = loot.items[i];
                        expect(item.amount).to.be.at.least(1);
                        expect(item.Price).to.be.at.least(1);
                        expect(item.PriceUnit).to.equal("gp");
                    }
                }
            });
    ...
    describe("service.generateTypeBLoot", function () {

        for (var budgetLowerBound = 0; budgetLowerBound < 100000; budgetLowerBound += 1000) {

            it("should generate type B loot for budgets " + budgetLowerBound + " to " + (budgetLowerBound + 950), function () {

                for (var budget = budgetLowerBound; budget <= budgetLowerBound + 950; budget += 5) {
                    var loot = service.generateTypeBLoot(budget);
                    expect(loot.coins.pp).to.be.at.least(0);
                    expect(loot.coins.gp).to.be.at.least(0);
                    expect(loot.coins.sp).to.be.at.least(0);
                    expect(loot.coins.cp).to.be.at.least(0);
    ...
    describe("service.generateTypeFLoot", function () {

        for (var budgetLowerBound = 0; budgetLowerBound < 1000000; budgetLowerBound += 1000) {

            it("should generate type F loot for budgets " + budgetLowerBound + " to " + (budgetLowerBound + 950), function () {

                for (var budget = budgetLowerBound; budget <= budgetLowerBound + 950; budget += 50) {
                    var loot = service.generateTypeFLoot(budget);
