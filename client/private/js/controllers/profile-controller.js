    ...
    function ($scope, $http, $rootScope) {
        ...
        $scope.changePassword = function () {
            ...
            if ($scope.newPassword1 !== $scope.newPassword2) {
                ...
                $http.post(HOST_URL[window.location.host] + 'api/change-password', { oldPassword: oldPassword, newPassword: newPassword1}, {withCredentials: true})
        ...
        $scope.changeUserData = function () {
            ...
            $scope.confirmedPassword = undefined;
            $http.post(HOST_URL[window.location.host] + 'api/change-user-data', { username: $scope.username, email: $scope.email, fullname: $scope.fullname, password: confirmedPassword }, {withCredentials: true})
                .success(function (response) {
                    if (response.error) {
                        errorAlert(response.error);
                    } else {
