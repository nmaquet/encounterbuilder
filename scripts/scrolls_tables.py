...
def parseTable(text):
    ...
    for line in text.splitlines():
        ...
        values.append(scroll(name, price))
    return "return rangeIn100(%s, %s);" % (json.dumps(upperBounds[:-1]), json.dumps(values));

if __name__ == "__main__":
    common_arcane_levels = []
    uncommon_arcane_levels = []
    common_divine_levels = []
    uncommon_divine_levels = []
    for i in range(10):
        common_arcane_levels.append('"%d" : function() { %s }' % (i, parseTable(common_arcane[i])))
    for i in range(10):
        uncommon_arcane_levels.append('"%d" : function() { %s }' % (i, parseTable(uncommon_arcane[i])))
    for i in range(10):
        common_divine_levels.append('"%d" : function() { %s }' % (i, parseTable(common_divine[i])))
    for i in range(10):
        uncommon_divine_levels.append('"%d" : function() { %s }' % (i, parseTable(uncommon_divine[i])))
    string = """var random_scroll = {
                   common : {
                        arcane : {
                            %s
                        },
           ...
           """ % (",\n".join(common_arcane_levels),
                  ",\n".join(common_divine_levels),
                  ",\n".join(uncommon_arcane_levels),
                  ",\n".join(uncommon_divine_levels))
    with open("../data/items/scrolls_tables.js", "w") as f:
        f.write(string)
